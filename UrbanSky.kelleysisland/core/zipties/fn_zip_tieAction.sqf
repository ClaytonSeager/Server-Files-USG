...
if (side player != civilian || {isNull cursorObject} || (player distance cursorObject) > 3 || {cursorObject getVariable "ziptied")} || {player == cursorObject} || {!isPlayer cursorObject}) exitWith {};
...
if ((playerSide isEqualTo civilian && {(LIFE_SETTINGS(getNumber, "ziptie_need_rebel") isEqualTo 1)}) && {!license_civ_rebel}) exitWith {
	hint "You need rebel training for this action.";
};
