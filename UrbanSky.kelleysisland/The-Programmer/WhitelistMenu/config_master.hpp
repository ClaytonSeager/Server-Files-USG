...
class Max_Settings_WhitelistMenu {
    ...
    cop_level_data[] = {"cops",10,"coplevel"}; // Format is : {"Category class name (below)",Maximum Level,"Column name in database"}
    ...
    class categories { // One class inside = One button = One licenses list
        class companies {
            ...
            condition_addLicense = "(call life_adminlevel) >= 2"; // Condition which allow or not to add the license
            condition_removeLicense = "(call life_adminlevel) >= 2"; // Condition which allow or not to remove the license
            licenses[] = {
				...
				{"Motorcycle Dealer","bike"},
				{"Tow Trucker Driver","tow"},
				{"Car Dealer","sport"},
				{"Repair Man","repair"},
                {"Boat licence","boat"},
				{"SurPlusSHop","SurplusShop"}
            };
        };

        class gangsters {
            displayName = "STR_CATEGORY_GANGSTERS";
            condition_addLicense = "(call life_adminlevel) >= 2";
            condition_removeLicense = "(call life_adminlevel) >= 2";
            licenses[] = {};
        };
